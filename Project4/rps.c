...
/*
 * This time, instead of using srand, I created a function that would
 * make the read() system call on /dev/rps to get the random number.
 */
char getRand(){
	char result;
	int x = open("/dev/rps", O_RDONLY);
	int y = read(x, &result, 1);
	return result;	
}
int main() {
